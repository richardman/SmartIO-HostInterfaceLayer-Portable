...
#define CALLBACK(h, cb, argtype)    if (cb) SmartIO__RegisterCallback(h, (void (*)(void))cb, argtype);
...
unsigned char *SmartIO_ReadEEPROM(uint16_t address, uint16_t length)
    {
    return (unsigned char *)(unsigned)APICall(_3ARGS, SMARTIO_SYS_EEPROM_READ, address, length);
    }
...
char *SmartIO_GetPhoneTime(void)
    {
    return (char *)(unsigned)APICall(_1ARG, SMARTIO_SYS_PHONE_TIME);
    }
...
static uint32_t APICall(unsigned argmask, ...)
    ...
    for (int i = 0; i < 8; i++)
        ...
        if (mask == 0b01)
            ...
            {
            ulongarg = va_arg(args, unsigned long);
            Command[index++] = (uint16_t)ulongarg;
            Command[index] = (uint16_t)(ulongarg >> 16);
            }
        index++;
        argmask >>= 2;
        }
