...
typedef void (*PFN_ARG_VOID)(void);
typedef void (*PFN_ARG_STRING)(unsigned char *);
typedef void (*PFN_ARG_ONE)(int16_t);
typedef void (*PFN_ARG_TWO)(int16_t, uint16_t);
typedef void (*PFN_ARG_THREE)(int16_t, uint16_t, uint16_t);
typedef void (*PFN_ARG_FOUR)(int16_t, uint16_t, uint16_t, uint16_t);

typedef struct callback {
    uint8_t argtype;
    uint16_t handle;
    void (*func)(void);         // "func" may take different arguments, must be casted before calling
} CALLBACK_ENTRY;
...
uint32_t SmartIO__SendBytes(unsigned char *sendbuf, int sendlen)
    ...
    if (is_syscall)
        ...
        if (op >= __SMARTIO_SYS_MULTIBYTE_RETURN)
            ...
            handle |= (uint32_t)FETCH_WORD(replybuf, 2) << 16;
        }
...
void SmartIO__RegisterCallback(uint16_t handle, void (*func)(void), int argtype)
    {
    ...
    if (replylen > 0)
        ...
        if (p == 0)
            ...
            case SMARTIO_ARG_STRING:
                replybuf[replylen] = 0;
                ((PFN_ARG_STRING)p->func)(&replybuf[2]);
                break;
            case SMARTIO_ARG_ONE:
                ((PFN_ARG_ONE)p->func)(*(uint16_t *)&replybuf[2]);
                break;
            case SMARTIO_ARG_TWO:
                ((PFN_ARG_TWO)p->func)(*(uint16_t *)(&replybuf[2]), *(uint16_t *)(&replybuf[4]));
                break;
            case SMARTIO_ARG_THREE:
                ((PFN_ARG_THREE)p->func)(*(uint16_t *)(&replybuf[2]), *(uint16_t *)(&replybuf[4]), *(uint16_t *)(&replybuf[6]));
                break;
            case SMARTIO_ARG_FOUR:
                ((PFN_ARG_FOUR)p->func)(*(uint16_t *)(&replybuf[2]), *(uint16_t *)(&replybuf[4]), *(uint16_t *)(&replybuf[6]), *(uint16_t *)(&replybuf[8]));
                }
            }
        }
    else if (replylen == 0)
